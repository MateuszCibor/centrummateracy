            ...
            {if $product.images}
                {foreach from=$product.images item=image name=covers}
                    <div class="product-lmage-large text-center">
                        ...
                        {if $product.images}<a class="expander"     data-image-large-src="{$image.large.url}" data-toggle="modal" data-target="#product-modal">{if $iqitTheme.pp_zoom == 'inner'}<span><i class="fa fa-expand" aria-hidden="true"></i></span> {/if}</a>{/if}
                        <img
