...
class SimpleBlogPost extends ObjectModel
{
    ...
    public static function getUrlRewriteInformations($id_simpleblog)
    {
        $sql = 'SELECT l.`id_lang`, c.`link_rewrite`
                FROM `'._DB_PREFIX_.'simpleblog_post_lang` AS c
    ...
    public static function getPageLink($page_nb, $type = false, $rewrite = false, $id_lang = null)
    {
        ...
        if ($page_nb > 1) {
            return $context->link->getModuleLink('ph_simpleblog', 'page', array_merge($params, $additionalParams), null,  $id_lang);
        } else {
            return $context->link->getModuleLink('ph_simpleblog', 'list', $additionalParams, null,  $id_lang);
        }
    }
    ...
    public function getAlternativeLangsPostUrl(){

         $sql = 'SELECT l.`id_lang`, c.`link_rewrite`
                FROM `'._DB_PREFIX_.'simpleblog_post_lang` AS c
                LEFT JOIN  `'._DB_PREFIX_.'lang` AS l ON c.`id_lang` = l.`id_lang`
                WHERE c.`id_simpleblog_post` = '.(int) $this->id_simpleblog_post.'
                AND l.`active` = 1';
        ...
        foreach ($results as $result) {
             $postRewrites[$result['id_lang']] = $result['link_rewrite'];
        }
        ...
        foreach ($languages as $lang) {
            ...
            $url = Context::getContext()->link->getModuleLink(
                ...
                'single',
                array(
                    'rewrite' => $postRewrites[$lang['id_lang']],
                    'sb_category' => SimpleBlogCategory::getRewriteByCategory($this->id_simpleblog_category, $lang['id_lang'])
                ), 
                null,$lang['id_lang']
            );
